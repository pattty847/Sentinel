...
- **Key classes/files:**
  ...
  - `execution/OrderStateMachine.{hpp,cpp}` — manages transitions `PENDING_NEW → NEW → PARTIALLY_FILLED → FILLED → DONE/CANCELLED`.
...
- `CoinbaseModels.hpp` defines:
  - `enum class OrderSide { Buy, Sell }`, `enum class OrderType { Market, Limit, Stop, StopLimit, TrailingStop }`, `enum class OrderStatus { PendingNew, New, PartiallyFilled, Filled, Canceled, Rejected, Done }`.
...
- Implement `CoinbaseModels` and `Serialization` with exhaustive unit tests using recorded Coinbase payloads.
...
- Add orchestrators for iceberg, bracket, OCO, trailing stop, and TWAP; expand into native child orders with linkage.
...
- **WS disconnects:** heartbeat watchdog + backoff reconnect; persist pending actions until reconnected.
- **Order/portfolio divergence:** periodic account sync plus idempotent application of WS deltas.
