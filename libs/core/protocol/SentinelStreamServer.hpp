...
private:
    ...
    // Trading/algo broadcast: each session registers a send callback
    using TradingBroadcastFn = std::function<void(const std::string&)>;
    std::mutex m_tradingBroadcastMutex;
    std::vector<std::pair<uint64_t, TradingBroadcastFn>> m_tradingBroadcasters;
    std::atomic<uint64_t> m_nextBroadcasterId{0};
public:
    uint64_t registerTradingBroadcaster(TradingBroadcastFn fn);
