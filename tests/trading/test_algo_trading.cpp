...
TEST(TradingEngineLimitTest, LimitBuyFillsWhenPriceCrosses) {
    ...
    cmd.side       = trading::OrderSide::Buy;
    cmd.orderType  = trading::OrderType::Limit;
    cmd.limitPrice = 100.0;  // buy at or below 100
    cmd.qty        = 1.0;
...
TEST(AvendellaMM, QuotePricesRespectSpread) {
    ...
    p.spreadBps = 10.0;  // 10 bps half-spread → bid = 99.9, ask = 100.1
    ...
    for (const auto& c : cmds) {
        if (c.side == trading::OrderSide::Buy)  bidPrice = c.limitPrice;
        if (c.side == trading::OrderSide::Sell) askPrice = c.limitPrice;
    }
    // 10 bps of 100 = 0.10
    EXPECT_NEAR(bidPrice, 99.9, 1e-6) << "Bid should be mid minus half-spread";
    EXPECT_NEAR(askPrice, 100.1, 1e-6) << "Ask should be mid plus half-spread";
}
